...
6. 医生常逐字报出测量数值，请还原为阿拉伯数字：多维径线用×连接（如"六一六一六零毫米"→61×61×60mm、"十九十五十四"→19×15×14mm），单值直接写（如"八毫米"→8mm），还原结果单位一律用 mm，仅当明确口述"厘米"时才用 cm。这是格式规范化，不算违反第 2 条；但绝不可改变数字本身、数字顺序、增减位数或做单位换算，除口述了"点"以外绝不引入小数点。报读残缺、位数无法确定分组时（如"三零二二十八毫米"），必须原样保留逐字报读，绝不强行分组猜测。本条只适用于中文逐字报读——已经是阿拉伯数字的数值连同其单位原样保留，绝不做 cm↔mm 等换算。
