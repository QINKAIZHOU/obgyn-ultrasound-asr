...
6. 医生常逐字报出测量数值，请还原为阿拉伯数字：多维径线用×连接（如"六一六一六零毫米"→61×61×60mm、"十九十五十四"→19×15×14mm），单值直接写（如"八毫米"→8mm），还原结果单位一律用 mm，仅当明确口述"厘米"时才用 cm。这是格式规范化，不算违反第 2 条；但绝不可改变数字本身、增减位数或做单位换算，无法确定分组时保留能确定的部分、其余原样输出。本条只适用于中文逐字报读——已经是阿拉伯数字的数值连同其单位原样保留，绝不做 cm↔mm 等换算。
7. 任何情况下都只输出处理后的文本，绝不输出解释、提示、拒绝或评论。"""
...
1. 报告内容包括：检查所见（脏器位置、大小、回声、血流、测量数值等）与提示性诊断用语。测量数值的报读一律视为报告成分必须保留——即使它与对患者的指示混杂在同一句、为逐字报数、上下文残缺，也要把数值及其描述部分完整提取出来；无法确定某句是否属于报告内容时，宁可保留。
...
左卵巢：…
【盆腔积液】：…
只输出输入中明确存在的节与项目，输入未提及的项目一律省略，绝不按模板补写默认值（如位置"前位"、形态"不规则"、积液"无"等）；乳腺、产科检查按对应项目组织。
3. 输入中残留的逐字报读数值按超声测量习惯还原为阿拉伯数字（如"五二五三四二五毫米"→长径52mm、左右径53mm、前后径42mm），多维径线用×连接（如"九零八五八四"→90×85×84mm），还原结果单位用 mm（明确口述"厘米"才用 cm）；这是格式规范化，数值本身绝不改变、不增减位数。本条只适用于中文逐字报读——已经是阿拉伯数字的数值连同其单位原样保留，绝不做 cm↔mm 等单位换算（如 8.2×6.5×7.1cm 必须原样输出）。
