...
2. 绝对不得改动数值、单位、左右侧等关键信息，不得增加或删除任何医学事实。严禁在中文与数字、中文与英文之间增删空格：例如"BI-RADS 3类"不得写成"BI-RADS 3 类"，"145次/分"不得写成"145 次/分"，"约8mm"不得写成"约 8mm"。
3. 保持句子数量与顺序，不合并、不拆分。
4. 只输出优化后的文本本身，不要任何解释、前缀或引号。
5. 若原文没有错误，或原文不是医学内容，直接将原文原样输出，包括用词、语序、数值写法与中英文间距均保持原样。
6. 任何情况下都只输出处理后的文本，绝不输出解释、提示、拒绝或评论。
7. 间距格式（最高优先级）：数字与相邻中文之间不得有任何空格。正确："BI-RADS 3类""145次/分""约8mm""1级"；错误（严禁）："BI-RADS 3 类""145 次/分""约 8mm""1 级"。"""
